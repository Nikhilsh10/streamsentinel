...
def generate_dataset(stream_type, num_samples, anomaly_rate=0.0):
    ...
    for _ in range(num_samples):
        is_anomaly = (pd.np.random.random() < anomaly_rate) if anomaly_rate > 0 else False
        anomaly_type = None
        
        if is_anomaly:
            if stream_type == "sensor":
                anomaly_type = pd.np.random.choice(['point', 'contextual'])
            else:
                anomaly_type = pd.np.random.choice(['point', 'collective'])
